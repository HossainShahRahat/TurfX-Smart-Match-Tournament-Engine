  ...
  useEffect(() => {
    ...
    async function loadFeed() {
      try {
        ...
        if (active) {
          setPosts((current) =>
            page === 1 ? payload.data : [...current, ...payload.data]
          );
          setError("");
        }
      } catch (loadError) {
      ...
      <section className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top,_rgba(15,118,110,0.18),_transparent_30%),linear-gradient(180deg,_rgba(15,23,42,0.96),_rgba(2,6,23,0.96))] p-6 shadow-2xl shadow-black/30">
        ...
        <div className="mt-8 flex items-center justify-center">
          <button
            ...
            disabled={loading}
            className="rounded-full border border-white/10 px-6 py-3 text-sm text-slate-200 transition hover:border-brand/40 disabled:opacity-60"
          >
